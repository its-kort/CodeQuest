...
public class l3 implements Initializable{
    ...
    private void RESTART(ActionEvent event) {
        Stage primaryStage = (Stage) exit.getScene().getWindow();
        try {
            Parent root = FXMLLoader.load(getClass().getResource(STYLE.LEVEL.A3_L3)); 
            Scene scene = new Scene(root, screenSize.getWidth(), screenSize.getHeight());
            primaryStage.setScene(scene);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void levelComplete(Button button) {
        if (CURRENT_QUESTION > CONFIG.TOTAL_QUESTIONS) {
            ...
            FXMLLoader loader  = new FXMLLoader(getClass().getResource(STYLE.LEVEL_COMPLETE.A3_L3)); 
            try {
